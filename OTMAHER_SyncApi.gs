 ...
 * Yalnız PARAMETRELER, BIRIM_FIYATLAR ve RECETELER okunur.
 * Hiçbir hücreye yazmaz; Adisyo/API anahtarlarını dışarı vermez.
 */
function doGet(e) {
  try {
    const action = String((e && e.parameter && e.parameter.action) || 'azura').trim();
    if (action !== 'azura') return otmaherJson_({ok:false,error:'Geçersiz action'});
    ...
    if (!paramsSheet || !pricesSheet || !recipesSheet) {
      throw new Error('Gerekli OTMAHER sekmelerinden biri bulunamadı.');
    }

    const p = paramsSheet.getRange(1,1,Math.max(1,paramsSheet.getLastRow()),2).getValues();
    const paramMap = {};
    p.slice(1).forEach(r => { if (r[0] !== '') paramMap[String(r[0]).trim()] = r[1]; });

    const priceRows = pricesSheet.getRange(1,1,Math.max(1,pricesSheet.getLastRow()),3).getValues();
    const prices = {};
    priceRows.slice(1).forEach(r => {
      if (!r[0]) return;
    ...
    const recipeRows = lastRecipeRow > 1 ? recipesSheet.getRange(2,1,lastRecipeRow-1,19).getValues() : [];
    const recipes = recipeRows.filter(r => r[0] && r[1]).map(r => ({
      urun:String(r[0]).trim(), boy:String(r[1]).trim(),
      Bonfile:Number(r[2])||0, Pirzola:Number(r[3])||0, Kanat:Number(r[4])||0, 'Köfte':Number(r[5])||0,
      Marul:Number(r[6])||0, Domates:Number(r[7])||0, 'Soğan':Number(r[8])||0,
      Ekmek:Number(r[18])||0, Ayran:Number(r[17])||0
    }));

    const packBySize = {};
    recipeRows.filter(r => r[0] && r[1]).forEach(r => {
